...
const onCityChange = (e) => {
    ...
    form.city = selectedCity ? `${selectedCity.type} ${selectedCity.name}` : '';
    form.city_id = cityId;
};
...
<template>
    ...
    >
        <div v-if="show" class="fixed inset-0 z-[90] flex items-end md:items-center justify-center p-0 md:p-4">
            <div class="bg-white w-full max-w-lg rounded-t-2xl md:rounded-2xl shadow-2xl max-h-[90vh] flex flex-col">
                ...
                <form @submit.prevent="submitForm" class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <button v-for="lbl in ['Rumah', 'Kantor', 'Kos']" :key="lbl" type="button"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select @change="onProvinceChange" :value="selectedProvinceId"
                                class="w-full h-12 px-4 rounded-xl bg-gray-50 border-none ring-1 ring-gray-200 focus:ring-2 focus:ring-blue-600 transition-all font-medium text-gray-900 text-sm">
                        ...
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-1.5">Kota</label>
                            <select @change="onCityChange" :disabled="isLoadingCities || cities.length === 0"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input v-model="form.district" type="text" placeholder="Opsional"
                                class="w-full h-12 px-4 rounded-xl bg-gray-50 border-none ring-1 ring-gray-200 focus:ring-2 focus:ring-blue-600 transition-all font-medium text-gray-900 text-sm" />
                        </div>
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-1.5">Kode Pos</label>
                            <input v-model="form.postal_code" type="text" placeholder="Opsional"
                                class="w-full h-12 px-4 rounded-xl bg-gray-50 border-none ring-1 ring-gray-200 focus:ring-2 focus:ring-blue-600 transition-all font-medium text-gray-900 text-sm" />
                        </div>
                    </div>
