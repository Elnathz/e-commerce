                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-2 space-y-6">
                        ...
                        <div class="overflow-hidden bg-white shadow-sm ring-1 ring-slate-200 rounded-2xl p-6">
                            <h3 class="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">Informasi
                                Pengiriman</h3>

                            <div v-if="address" class="space-y-2 text-sm text-slate-700">
                                <p><span class="font-semibold text-slate-900">Penerima:</span> {{ address.recipient_name
                                    }}</p>
                                ...
                                <p class="mt-4"><span class="font-semibold text-slate-900">Kurir & Layanan:</span> <span
                                        class="uppercase font-bold">{{ order.courier }}</span> <span
                                        v-if="order.shipping_service"> - {{ order.shipping_service }}</span></p>
                                <p v-if="order.tracking_number"><span class="font-semibold text-slate-900">Nomor
                                        Resi:</span>
                                    <span class="font-mono bg-slate-100 px-2 py-1 rounded text-blue-700 font-bold">{{
                        ...
                        <!-- Daftar Item -->
                        <div class="overflow-hidden bg-white shadow-sm ring-1 ring-slate-200 rounded-2xl p-6">
                            <h3 class="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">Produk yang
                                Dipesan
                            </h3>
                            ...
                            <div class="space-y-4">
                                <div v-for="item in order.items" :key="item.id"
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-xs text-slate-500">Varian: {{ item.variant_name_snapshot }} |
                                            Berat:
                                            {{ item.weight_gram }}g</span>
                                        <span class="text-sm text-slate-600">{{ item.quantity }} x Rp {{
                    ...
                    <div class="space-y-6">
                        ...
                        <!-- Panel Aksi Status -->
                        <div class="overflow-hidden bg-white shadow-sm ring-1 ring-slate-200 rounded-2xl p-6">
                            <h3 class="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">Manajemen
                                Status
                            </h3>
                            ...
                            <div class="mb-6 flex items-center justify-between">
                                ...
                                <span
                                    :class="['px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide', getStatusClass(order.status)]">
                                    {{ order.status }}
                                </span>
                            </div>

                            <div v-if="order.status === 'paid'"
                                class="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-800 mb-4">
                                Pembayaran telah dikonfirmasi. Anda dapat mulai memproses pesanan ini.
                                <PrimaryButton @click="confirmProcess" type="button"
                                    class="mt-3 w-full justify-center !bg-blue-600 hover:!bg-blue-700 !rounded-lg">
                                    Proses Pesanan Sekarang
                                </PrimaryButton>
                            </div>

                            <div v-if="order.status === 'processing'"
                                class="bg-purple-50 border border-purple-100 rounded-xl p-4 mb-4">
                                <p class="text-sm text-purple-800 mb-3 font-medium">Pesanan sedang diproses. Masukkan
                                    nomor resi
                                    jika barang sudah diserahkan ke kurir.</p>
                                <form @submit.prevent="submitShip" class="space-y-3">
                                    <div>
                                        ...
                                        <TextInput id="tracking_number" type="text" class="mt-1 block w-full"
                                            v-model="formShip.tracking_number" required />
                                        <InputError class="mt-2" :message="formShip.errors.tracking_number" />
                                    </div>
                                    <PrimaryButton :disabled="formShip.processing" type="submit"
                                        class="w-full justify-center !bg-purple-600 hover:!bg-purple-700 !rounded-lg">
                                        Kirim & Simpan Resi
                                    </PrimaryButton>
                                </form>
                            </div>
                            ...
                            <div v-if="order.status === 'shipped'"
                                class="bg-indigo-50 border border-indigo-100 rounded-xl p-4 text-sm text-indigo-800">
                                Barang telah dikirim dengan nomor resi <span class="font-bold">{{ order.tracking_number
                                    }}</span>.<br /> Menunggu konfirmasi penerimaan dari pelanggan.
                            </div>
                            ...
                            <div v-if="order.status === 'cancelled' || order.status === 'refunded'"
                        ...
                        <!-- Rincian Pembayaran -->
                        <div class="overflow-hidden bg-white shadow-sm ring-1 ring-slate-200 rounded-2xl p-6">
                            <h3 class="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">Rincian
                                Harga</h3>
                            ...
                            <div class="space-y-2 text-sm text-slate-700">
                                ...
                                <div
                                    class="flex justify-between pt-2 border-t border-slate-100 mt-2 text-base font-bold text-slate-900">
                            ...
                            <div class="mt-6 pt-4 border-t border-slate-100">
                                ...
                                <p class="text-sm font-bold text-slate-900">{{ order.payment_method || 'Belum dipilih'
                                    }}</p>
                                <p v-if="order.paid_at" class="text-xs text-slate-500 mt-1">Dibayar pada: {{ new
                                    Date(order.paid_at).toLocaleString('id-ID') }}</p>
                            </div>
                        </div>
        ...
        <Modal :show="confirmingProcess" @close="closeModal">
            <div class="p-6">
                ...
                <p class="text-sm text-slate-600 mb-6">
                    Apakah Anda yakin ingin memproses pesanan ini? Status akan berubah menjadi <strong>Diproses</strong>
                    dan
                    Anda tidak bisa membatalkannya setelah ini.
                </p>
